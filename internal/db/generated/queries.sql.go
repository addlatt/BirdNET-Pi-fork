...
const getSpeciesRanking = `-- name: GetSpeciesRanking :many
...
type GetSpeciesRankingRow struct {
	SciName          string          `db:"sci_name" json:"sci_name"`
	ComName          string          `db:"com_name" json:"com_name"`
	DetectionCount   int64           `db:"detection_count" json:"detection_count"`
...
func (q *Queries) GetSpeciesRanking(ctx context.Context) ([]GetSpeciesRankingRow, error) {
	rows, err := q.db.QueryContext(ctx, getSpeciesRanking)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetSpeciesRankingRow
	for rows.Next() {
		var i GetSpeciesRankingRow
		if err := rows.Scan(
			&i.SciName,
			&i.ComName,
			&i.DetectionCount,
