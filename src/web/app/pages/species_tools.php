...
$db = isset($_GET['delete']) ? get_db_writable() : get_db();

/* Paths / lists */
$base_symlink   = $home . '/BirdSongs/Extracted/By_Date';
$base           = realpath($base_symlink);

$confirm_file   = PROJECT_ROOT . '/scripts/confirmed_species_list.txt';
$exclude_file   = PROJECT_ROOT . '/scripts/exclude_species_list.txt';
$whitelist_file = PROJECT_ROOT . '/scripts/whitelist_species_list.txt';
...
<?php while ($row = $result->fetchArray(SQLITE3_ASSOC)) {
  ...
  $chart_cell = sprintf("<img style='height: 1em;cursor:pointer;float:unset;display:inline' title='View species stats' onclick=\"generateMiniGraph(this, '%s', 180)\" width=25 src='/assets/images/chart.svg'>", $comnamegraph);

  $identifier_js = addslashes($identifier);
  $identifier_sci_js = addslashes($identifier_sci);

  $confirm_cell = $is_confirmed
    ? "<img style='cursor:pointer;max-width:12px;max-height:12px' src='/assets/images/check.svg' onclick=\"toggleSpecies('confirmed','{$identifier_sci_js}','del')\">"
    : "<span class='circle-icon' onclick=\"toggleSpecies('confirmed','{$identifier_sci_js}','add')\"></span>";

  $excl_cell = $is_excluded
    ? "<img style='cursor:pointer;max-width:12px;max-height:12px' src='/assets/images/check.svg' onclick=\"toggleSpecies('exclude','{$identifier_js}','del')\">"
    : "<span class='circle-icon' onclick=\"toggleSpecies('exclude','{$identifier_js}','add')\"></span>";

  $white_cell = $is_whitelisted
    ? "<img style='cursor:pointer;max-width:12px;max-height:12px' src='/assets/images/check.svg' onclick=\"toggleSpecies('whitelist','{$identifier_js}','del')\">"
  ...
  echo "<tr data-comname=\"{$common}\" data-sciname=\"{$scient}\">"
     ...
     . "<td><img style='cursor:pointer;max-width:20px' src='/assets/images/delete.svg' onclick=\"deleteSpecies('".addslashes($row['Sci_Name'])." + ".addslashes($row['Com_Name'])."')\"></td>"
     . "</tr>";
} ?>
    </tbody>
  </table>
