 ...
 *
 * This file expects to be included via bootstrap.php which defines:
 * - PROJECT_ROOT, DB_PATH, CONFIG_FILE, MODEL_PATH, HOME_PATH, etc.
 */

// Ensure we're loaded via bootstrap
if (!defined('APP_STARTED')) {
    die('This file must be included via bootstrap.php');
}
...
function get_user() {
    // Use bootstrap constant if available, otherwise get from config
    if (defined('BIRDNET_USER')) {
        return BIRDNET_USER;
    }
    $config = get_config();
    return $config['BIRDNET_USER'] ?? 'pi';
}

function get_home() {
    // Use bootstrap constant if available
...
function is_authenticated() {
    ...
    $config = get_config();
    return ($_SERVER['PHP_AUTH_PW'] == $config['CADDY_PWD'] && $_SERVER['PHP_AUTH_USER'] == 'birdnet');
}
...
function debug_log($message) {
    ...
    $log_path = defined('PUBLIC_PATH') ? PUBLIC_PATH : $_SERVER['DOCUMENT_ROOT'];
    error_log($message . "\n", 3, $log_path . "/debug_log.log");
}

function get_com_en_name($sci_name) {
    static $_labels_flickr = null;
    if ($_labels_flickr === null) {
        $labels_path = (defined('MODEL_PATH') ? MODEL_PATH : get_home() . "/BirdNET-Pi/model") . "/l18n/labels_en.json";
        $_labels_flickr = json_decode(file_get_contents($labels_path), true);
    }
    return $_labels_flickr[$sci_name] ?? $sci_name;
}
 ...
 * Uses absolute path from DB_PATH constant (defined in bootstrap.php)
 */
function get_db() {
    static $_db = null;
    if ($_db === null) {
        // Use absolute path from bootstrap constant
        $_db = new SQLite3(DB_PATH, SQLITE3_OPEN_READONLY);
        $_db->busyTimeout(1000);
    }
    return $_db;
}

function get_db_writable() {
    // Use absolute path from bootstrap constant - writable connection
...
class Wikipedia extends ImageProvider {
    ...
    protected function get_from_source($sci_name) {
        ...
        foreach ($metadata['query']['pages'] as $page) {
            ...
            if (($page["imageinfo"][0]["width"] ?? 0) > 1024) {
...
/**
 * Get color scheme CSS path
 * Returns path relative to web root for use in href
 */
function get_color_scheme() {
    $config = get_config();
    if (strtolower($config['COLOR_SCHEME'] ?? '') === 'dark') {
        return 'dark';  // Return identifier, let caller build full path
    }
    return 'light';
}

/**
 * Get the CSS file path for the current color scheme
 */
function get_color_scheme_css() {
    $scheme = get_color_scheme();
    if ($scheme === 'dark') {
        return '/assets/css/dark-style.css';
    }
